...
BUCKET = 'athena-sh-results'
OUTPUT_KEY = 'part2/results_2.ndjson'
SKIPPED_KEY = 'part2/skipped_part2.json'
PROCESSED_DIR = 'part2/log2/'
...
def process_sha1(args):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            r = session.get(f"{S3_URL}{sha1}", timeout=15)
            if r.status_code in (429, 503):
                consecutive_throttle[0] += 1
                log(f"THROTTLE #{consecutive_throttle[0]}: {r.status_code} for {sha1[:8]}...")
                time.sleep(min(MAX_BACKOFF, 2 ** consecutive_throttle[0]))
                continue
            else:
            ...
            try:
                content = subprocess.run(["pigz", "-d"], input=r.content, capture_output=True,
                                         timeout=10).stdout.decode("utf-8", errors="ignore")
            except Exception as e:
                return {"skipped": sha1, "reason": f"decompress_fail"}
            if not content:
                return None
    ...
    for line in f:
        ...
        if len(batch) >= BATCH_SIZE:
            ...
            if counter % SAVE_EVERY == 0:
                ...
                processed_log = []
                log(f"Checkpoint {chunk_num} saved!")
...
log(f"DONE! {counter} | Found: {found} | Skip: {len(skipped)} | {(time.time() - start_time) / 3600:.1f}h")